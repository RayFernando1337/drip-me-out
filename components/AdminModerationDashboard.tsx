  ...
  return (
    <div className="space-y-8">
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground">Review and moderate featured images in the public gallery</p>
      </div>
          ...
          <div key={image._id} className="group">
            <div
              className={`bg-card border transition-all duration-200 overflow-hidden rounded-xl shadow-sm ${
                (image as any).isDisabledByAdmin
            ...
            >
              ...
              <div className="p-4 space-y-3">
                <div className="text-xs text-muted-foreground">User: {(image as any).userId || "Unknown"}</div>

                {(image as any).isDisabledByAdmin && (image as any).disabledByAdminReason && (
                  <div className="text-xs text-red-600 bg-red-50 dark:bg-red-900/20 p-2 rounded">
                    <strong>Disabled:</strong> {(image as any).disabledByAdminReason}
                  </div>
                )}
      ...
      {featuredImages?.continueCursor && (
        <div className="flex justify-center">
          <Button
            onClick={() =>
              setPaginationOpts((prev) => ({ numItems: 20, cursor: featuredImages.continueCursor }))
            }
            variant="ghost"
          >
            Load More Images
