...
## IMPORTANT UPDATE (2025-10-18): Simplified Architecture

**Cost analysis revealed the server-side re-encoding was over-engineered for current scale (0-10K users).**

### What Changed

- **REMOVED:** `/api/encode-webp` route and server-side sharp re-compression
- **REMOVED:** `sharp` and `image-size` npm dependencies
- **SIMPLIFIED:** Gemini output now stored directly to Convex without re-encoding
- **OPTIMIZED:** Next.js Image configuration (31-day cache TTL, single format, optimized device sizes)

### Why This Simplification Works

1. **Gemini already optimizes:** AI output is ~1.5MB, already reasonable size
2. **Vercel handles transformations:** Next.js Image Optimization at edge is more cost-effective at small scale
3. **Client prep is sufficient:** User uploads already compressed to WebP ≤3MB via browser
4. **Cost savings:** Eliminates Convex bandwidth overhead and complexity

### Cost Comparison (1,000 users/month)

- **Previous approach:** ~$15/month (mostly Convex bandwidth)
- **Simplified approach:** ~$10/month (mostly free tier)
- **Savings:** ~33% reduction + less code to maintain

### When to Revisit

Re-introduce pre-optimization when:
